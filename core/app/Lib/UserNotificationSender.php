...
class UserNotificationSender
{
     ...
     */
    public function notificationToAll($request, $userType = 'User')
    {
        ...
        $userQuery      = $this->getUserQuery($request, $model);
        ...
        return $this->manageSessionForNotification($totalUserCount, $request, $userType);
    }
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function notificationToSingle($request, $userId, $userType = 'User')
    {
        if (!$this->isTemplateEnabled($request->via)) {
            return $this->redirectWithNotify('warning', 'Default notification template is not enabled');
        }
     ...
     */
    private function getUserQuery($request, $model)
    {
        $scope = $request->being_sent_to;
        return $model::oldest()->active()->$scope();
    }
    ...
    private function sendNotifications($users, $request, $imageUrl, $isSingleNotification = false)
    {
        if (!$isSingleNotification) {
            foreach ($users as $user) {
                if ($user->is_allow_promotional_notify == Status::ENABLE) {
                    notify($user, 'DEFAULT', [
            ...
            if ($users->is_allow_promotional_notify == Status::ENABLE) {
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    private function manageSessionForNotification($totalUserCount, $request, $userType)
    {
        if (session()->has('SEND_NOTIFICATION')) {
        ...
        if ($sessionData['total_sent'] >= $totalUserCount) {
            ...
            $url     = route("admin." . strToLower($userType) . "s.notification.all");
        } else {
            ...
            $url     = route("admin." . strToLower($userType) . "s.notification.all") . "?email_sent=yes";
        }
