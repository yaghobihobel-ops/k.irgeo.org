...
class AgentDepositController extends Controller
{
    public function pending()
    {
        ...
        $deposits    = $baseQuery->with(['agent', 'gateway'])->paginate(getPaginate());
        return view('admin.agent_deposit.log', compact('pageTitle', 'deposits'));
    }
    ...
    public function deposit()
    {
        ...
        return view('admin.agent_deposit.log', compact('pageTitle', 'deposits', 'widget'));
    }
    ...
    {
        $deposit   = Deposit::where('id', $id)->with(['agent', 'gateway'])->firstOrFail();
        $pageTitle = @$deposit->agent->username . ' requested ' . showAmount($deposit->amount);
    ...
    {
        $deposit = Deposit::where('id', $id)->where('status', Status::PAYMENT_PENDING)->agentDeposit()->firstOrFail();

        PaymentController::userDataUpdate($deposit, true);

        $notify[] = ['success', 'Add Money request approved successfully'];

        return to_route('admin.agent.deposit.pending')->withNotify($notify);
    }
    ...
    public function reject(Request $request)
    {
        ...
        $deposit = Deposit::where('id', $request->id)->where('status', Status::PAYMENT_PENDING)->agentDeposit()->firstOrFail();
    ...
    {
        $baseQuery = Deposit::$scope()->searchable(['trx', 'agent:username'])->agentDeposit()->dateFilter();
        $request   = request();
