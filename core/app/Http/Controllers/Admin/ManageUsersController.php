...
class ManageUsersController extends Controller
{
    ...
    {
        $pageTitle = 'Account Deleted Users';
    ...
    {
        $baseQuery  = User::$scope()->searchable(['email', 'username', 'firstname', 'lastname', 'mobile'])->dateFilter()->filter(['status'])->orderBy('id', getOrderBy());
    ...
    public function detail($id)
    {
        ...
        $widget['total_send_money']           = SendMoney::where('sender_id', $user->id)->sum('amount');
        $widget['total_payment']              = MakePayment::where('user_id', $user->id)->sum('amount');
        $widget['total_mobile_recharge']      = MobileRecharge::where('user_id', $user->id)->approved()->sum('amount');
        $widget['total_bank_transfer']        = BankTransfer::where('user_id', $user->id)->approved()->sum('amount');
        $widget['total_microfinance_payment'] = Microfinance::where('user_id', $user->id)->approved()->sum('amount');
        $widget['total_utility_bill']         = UtilityBill::where('user_id', $user->id)->approved()->sum('amount');
        $widget['total_education_fee']        = EducationFee::where('user_id', $user->id)->approved()->sum('amount');
        $widget['total_donation']             = Donation::where('user_id', $user->id)->sum('amount');
