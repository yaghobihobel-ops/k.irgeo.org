        ...
        <div class="card custom--card">
            <div class="card-header">
                <div class="payment-info-top">
                    ...
                    <div class="content">
                        <h4 class="title">@lang('Send Money Successful!')</h4>
                        <p class="desc">@lang('Your send money has been successfully done.')</p>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <div class="payment-info">
                    <div class="payment-info-item">
                        <span class="title">@lang('TRX')</span>
                        <span class="desc">{{ $sendMoney->trx }}</span>
                    </div>
                    <div class="payment-info-item">
                        <span class="title">@lang('Time')</span>
                        <span class="desc">{{ showDateTime($sendMoney->created_at) }}</span>
                    </div>
                    <div class="payment-info-item">
                        <span class="title">@lang('Amount')</span>
                        <span class="desc">{{ showAmount($sendMoney->amount) }}</span>
                    </div>
                    <div class="payment-info-item">
                        <span class="title">(+) @lang('Charge')</span>
                        <span class="desc">{{ showAmount($sendMoney->charge) }}</span>
                    </div>
                    <div class="payment-info-item total">
                        <span class="title">@lang('Total')</span>
                        <span class="desc">{{ showAmount($sendMoney->total_amount) }}</span>
                    </div>
                </div>

                <div class="payment-balance mb-4 mb-sm-5">
                    <span class="title">@lang('Post Balance')</span>
                    <span class="amount">{{ showAmount($sendMoney->sender_post_balance) }}</span>
                </div>

                <div class="payment-btn-wrapper flex-align gap-3 gap-lg-4">
                    <a href="{{ route('user.send.money.history') }}" class="btn btn--light flex-grow-1">
                        <span class="icon">
                        ...
                        @lang('Send Money History')
                    </a>

                    <a href="{{ route('user.send.money.pdf', $sendMoney->id) }}" class="btn btn--base flex-grow-1">
                        <span class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                fill="none">
                                <path
