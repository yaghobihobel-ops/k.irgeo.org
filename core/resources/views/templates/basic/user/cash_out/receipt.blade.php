        ...
        <div class="card custom--card">
            <div class="card-header">
                <div class="payment-info-top">
                    ...
                    <div class="content">
                        <h4 class="title">@lang('Cash Out Successful!')</h4>
                        <p class="desc">@lang('Your cash out has been successfully done.')</p>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <div class="payment-info">
                    <div class="payment-info-item">
                        ...
                        <span class="desc">{{ $cashOut->trx }}</span>
                    </div>
                    <div class="payment-info-item">
                        <span class="title">@lang('Time')</span>
                        <span class="desc">{{ showDateTime($cashOut->created_at) }}</span>
                    </div>
                    <div class="payment-info-item">
                        <span class="title">@lang('Amount')</span>
                        <span class="desc">{{ showAmount($cashOut->amount) }}</span>
                    </div>
                    <div class="payment-info-item">
                        <span class="title">(+) @lang('Charge')</span>
                        <span class="desc">{{ showAmount($cashOut->charge) }}</span>
                    </div>
                    <div class="payment-info-item total">
                        <span class="title">@lang('Total')</span>
                        <span class="desc">{{ showAmount($cashOut->total_amount) }}</span>
                    </div>
                </div>

                <div class="payment-balance mb-4 mb-sm-5">
                    <span class="title">@lang('Post Balance')</span>
                    <span class="amount">{{ showAmount($cashOut->user_post_balance) }}</span>
                </div>

                <div class="payment-btn-wrapper flex-align gap-3 gap-lg-4">
                    <a href="{{ route('user.cash.out.history') }}" class="btn btn--light flex-grow-1">
                        <span class="icon">
                        ...
                        @lang('Cash Out History')
                    </a>


                    <a href="{{ route('user.cash.out.pdf', $cashOut->id) }}" class="btn btn--base flex-grow-1">
                        <span class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                fill="none">
                                <path
