        ...
        <div class="container">
            <div class="account-form">
                ...
                <div class="account-form__content text-center">
                    <h2 class="account-form__title">@lang('Email Verification')</h2>
                    <p class="account-form__desc">
                        @lang('We have sent a code to')
                        <span class="number"> {{ showEmailAddress(auth()->user()->email) }}</span>
                    </p>
                </div>
                <form action="{{ route('user.verify.email') }}" method="POST" class="submit-form verify-form">
                    @csrf
                    <div class="otp-inner">
        ...
        var x = setInterval(function() {
            ...
            if (distance <= 0) {
                ...
                $(".resend-link").removeClass('cursor-select-none').attr('href',
                    "{{ route('user.send.verify.code', 'sms') }}");
