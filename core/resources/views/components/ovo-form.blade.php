<div class="row">
    @props(['noFileType' => false])
    @foreach ($formData as $data)
        @php
            $fieldValue = collect($filledData ?? [])->firstWhere('name', $data->name)->value ?? old($data->label);
            if ($data->type == 'file' && $noFileType) {
                continue;
            }
        @endphp
        <div class="col-md-{{ @$data->width ?? '12' }}">
            <div class="form-group">
                <label
                    class="form-label form--label @if ($data->is_required == 'required') required @endif">{{ __($data->name) }}
                    @if (@$data->instruction)
                ...
                @if ($data->type == 'text')
                    <input type="text" class="form-control form--control" name="{{ $data->label }}"
                        value="{{ $fieldValue }}" @if ($data->is_required == 'required') required @endif>
                ...
                @elseif($data->type == 'number')
                    <input type="number" class="form-control form--control" name="{{ $data->label }}"
                        value="{{ $fieldValue }}" step="any" @if ($data->is_required == 'required') required @endif>
                @elseif($data->type == 'textarea')
                    <textarea class="form-control form--control" name="{{ $data->label }}"
                        @if ($data->is_required == 'required') required @endif>{{ $fieldValue }}</textarea>
                @elseif($data->type == 'select')
                    <select class="form-select form--control select2" data-minimum-results-for-search="-1"
                        ...
                        @foreach ($data->options as $item)
                            <option value="{{ $item }}" @selected($item == $fieldValue)>{{ __($item) }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        @foreach ($data->options as $option)
                            <div class="form-check">
                                <input class="form-check-input" name="{{ $data->label }}[]" type="checkbox"
                                    ...
                                    @checked(in_array($option, $fieldValue ?? []))>
