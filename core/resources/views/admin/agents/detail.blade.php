...
@section('panel')
                    ...
                    <div class="user-detail">
                        <div class="user-detail__user">
                            <div class="user-detail__thumb">
                                <img class="fit-image" src="{{ $agent->image_src }}" alt="agent">
                            </div>
                            <div class="user-detail__user-info">
                                <h5 class="user-detail__name mb-1">{{ __($agent->fullname) }}</h5>
                                <p class="user-detail__username">{{ '@' . $agent->username }}</p>
                            </div>
                            <x-permission_check permission="login as agent">
                                <div class="login-user">
                                    <a target="_blank" href="{{ route('admin.agents.login', $agent->id) }}"
                            ...
                            <div class="col-md-6">
                                <ul class="user-detail__contact">
                                    ...
                                    <li class="item">
                                        ...
                                        <span>{{ __($agent->country_name) }}</span>
                                    </li>
                                </ul>
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="row g-0">
                            <div class="col-sm-6">
                                <div class="widget-card widget--success">
                                    <a href="{{ route('admin.report.agent.transaction') }}?search={{ $agent->username }}"
                                    ...
                                    <div class="widget-card-left">
                                        ...
                                        <div class="widget-card-content">
                                            ...
                                            <h6 class="widget-amount">
                                                {{ gs('cur_sym') }}{{ showAmount($agent->balance, currencyFormat: false) }}
                                                <span class="currency">
                                                    {{ __(gs('cur_text')) }}
                                                </span>
                                            </h6>
                            ...
                            <div class="col-sm-6">
                                <div class="widget-card widget--success">
                                    <a href="{{ route('admin.agent.deposit.list') }}?search={{ $agent->username }}"
                                        class="widget-card-link"></a>
                                    <div class="widget-card-left">
                                        <div class="widget-icon">
                            ...
                            <div class="col-sm-6">
                                <div class="widget-card widget--warning">
                                    <a href="{{ route('admin.withdraw.data.all') }}?search={{ $agent->username }}"
                                        class="widget-card-link"></a>
                                    <div class="widget-card-left">
                                        <div class="widget-icon">
            ...
            <form action="{{ route('admin.agents.update', [$agent->id]) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-3">
                        ...
                        <x-permission_check permission="update agent">
                            ...
                            <div class="col-lg-6 col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group input--group ">
                                        <span class="input-group-text mobile-code">+{{ $agent->dial_code }}</span>
                                        <input type="number" name="mobile" value="{{ $agent->mobile }}"
                                    ...
                                    <select name="country" class="form-control select2">
                                        @foreach ($countries as $key => $country)
                                            <option data-mobile_code="{{ $country->dial_code }}"
                                                value="{{ $key }}" @selected($agent->country_code == $key)>
                            ...
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
                                ...
                                <div class="form-check form-switch form-switch-success form--switch pl-0">
                                    <input class="form-check-input" type="checkbox" role="switch"
                                        id="email_verification" name="ev" @checked($agent->ev)>
                                </div>
                            </div>
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
                                ...
                                <div class="form-check form-switch form-switch-success form--switch pl-0">
                                    <input class="form-check-input" type="checkbox" role="switch"
                                        id="mobile_berification" name="sv" @checked($agent->sv)>
                                </div>
                            </div>
                            ...
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
                                ...
                                <div class="form-check form-switch form-switch-success form--switch pl-0">
                                    <input class="form-check-input" type="checkbox" role="switch" id="kyc_verification"
                                        name="kv" @checked($agent->kv == Status::KYC_VERIFIED)>
                                </div>
                            </div>
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
    ...
    <x-admin.ui.modal id="agentStatusModal">
        <x-admin.ui.modal.header>
            <div>
                ...
                @if ($agent->status == Status::AGENT_ACTIVE)
                    <small>@lang('If this agent is banned, they will no longer have access to their dashboard.')</small>
                @else
                    <small>
                        <span class=" text--info">@lang('Ban reason was'):</span>
                        <span>{{ __($agent->ban_reason) }}</span>
                    </small>
                @endif
            </div>
        ...
        <x-admin.ui.modal.body>
            <form action="{{ route('admin.agents.status', $agent->id) }}" method="POST">
                ...
                @else
                    <h4 class="mt-3 text-center text--warning">@lang('Are you sure to unban this agent?')</h4>
                @endif
                <div class="form-group">
                    @if ($agent->status == Status::AGENT_ACTIVE)
    ...
    <x-admin.ui.modal id="addSubModal">
        <x-admin.ui.modal.header>
            <div>
                ...
                <small class="modal-subtitle">@lang('Add funds to agent accounts by entering the desired amount below')</small>
            </div>
            <button type="button" class="btn-close close" data-bs-dismiss="modal" aria-label="Close">
                <i class="las la-times"></i>
            </button>
        ...
        <x-admin.ui.modal.body>
            <form method="POST" action="{{ route('admin.agents.add.sub.balance', $agent->id) }}">
                @csrf
...
@push('breadcrumb-plugins')
    <div class=" d-flex gap-2  flex-wrap">
        <x-permission_check permission="update agent balance">
        ...
        <x-permission_check permission="ban agent">
            @if ($agent->status == Status::AGENT_ACTIVE)
                <button type="button" class="flex-fill btn  btn--warning" data-bs-toggle="modal"
                    data-bs-target="#agentStatusModal">
                    <i class="las la-ban me-1"></i>@lang('Ban Agent')
                </button>
            @else
                <button type="button" class="flex-fill btn  btn--info" data-bs-toggle="modal"
                    data-bs-target="#agentStatusModal">
                    <i class="las la-ban me-1"></i>@lang('Unban Agent')
                </button>
            @endif
        </x-permission_check>
        <x-permission_check permission="view agent notifications">
            <a href="{{ route('admin.report.agent.notification.history') }}?search={{ $agent->username }}"
                class="flex-fill btn  btn--secondary">
                <i class="las la-bell me-1"></i>@lang('Notifications')
            </a>
        </x-permission_check>
        <x-permission_check permission="verify agent kyc">
            @if ($agent->kyc_data)
        ...
        (function($) {
            ...
            $(".balance-adjust").on('click', function(e) {
                ...
                if (act == 'add') {
                    ...
                    modal.find(".modal-subtitle").text("@lang('Add funds to agent accounts by entering the desired amount below')");
                } else {
                    modal.find(".modal-title").text("@lang('Subtract Balance')");
                    modal.find(".modal-subtitle").text("@lang('Subtract funds to agent accounts by entering the desired amount below')");
                }
