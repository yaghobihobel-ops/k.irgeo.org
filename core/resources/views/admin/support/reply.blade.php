...
@push('breadcrumb-plugins')
    <div class="d-flex gap-2 flex-wrap">
        ...
        @endif
        @if ($ticket->user_id != 0)
            <x-back_btn route="{{ route('admin.ticket.user.index') }}" />
        @elseif($ticket->agent_id != 0)
            <x-back_btn route="{{ route('admin.ticket.agent.index') }}" />
        @else
            <x-back_btn route="{{ route('admin.ticket.merchant.index') }}" />
        @endif
    </div>
@endpush
