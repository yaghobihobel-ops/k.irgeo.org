        ...
        <div class="col-xl-4 col-md-6">
            <x-admin.ui.card>
                <x-admin.ui.card.header>
                    <h4 class="card-title">
                        @lang('Add Money') @if ($deposit->method_code < 5000)
        ...
        @if ($details || $deposit->status == Status::PAYMENT_PENDING)
            <div class="col-xl-8 col-md-6">
                <x-admin.ui.card class="h-100">
                    <x-admin.ui.card.header>
                        <h4 class="card-title">@lang('Agent Deposit Information')</h4>
                    </x-admin.ui.card.header>
                    <x-admin.ui.card.body>
                        @if ($details != null)
                            ...
                            @endforeach
                            @if ($deposit->method_code < 1000)
                                @include('admin.agent_deposit.gateway_data', [
                        ...
                        @if ($deposit->status == Status::PAYMENT_PENDING)
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <x-permission_check permission="approve agent add money">
                                    ...
                                    <x-permission_check permission="reject agent add money">
            ...
            <form action="{{ route('admin.agent.deposit.reject') }}" method="POST">
                ...
                <p>
                    @lang('Are you sure to') <span class="fs-14">@lang('reject')</span> <span
                        class="fw-bold text--success">{{ showAmount($deposit->amount) }}</span> @lang('add money of')
                    <span class="fs-14">{{ @$deposit->agent->username }}</span>?
                </p>
