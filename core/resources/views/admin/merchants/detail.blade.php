...
@section('panel')
                    ...
                    <div class="user-detail">
                        <div class="user-detail__user">
                            <div class="user-detail__thumb">
                                <img class="fit-image" src="{{ $merchant->image_src }}" alt="merchant">
                            </div>
                            <div class="user-detail__user-info">
                                <h5 class="user-detail__name mb-1">{{ __($merchant->fullname) }}</h5>
                                <p class="user-detail__username">{{ '@' . $merchant->username }}</p>
                            </div>

                            <x-permission_check permission="login as merchant">
                                <div class="login-user">
                                    <a target="_blank" href="{{ route('admin.merchants.login', $merchant->id) }}"
                                        class="btn btn--primary">
                                        <i class="fas fa-sign-in-alt me-1"></i>
                                        <span>@lang('Login as Merchant')</span>
                                    </a>
                                </div>
                            </x-permission_check>
                        </div>
                            ...
                            <div class="col-md-6">
                                <ul class="user-detail__contact">
                                    ...
                                    <li class="item">
                                        ...
                                        <span>{{ $merchant->mobileNumber }}</span>
                                    </li>
                                    <li class="item">
                                        <span>@lang('Country'): </span>
                                        <span>{{ __($merchant->country_name) }}</span>
                                    </li>
                                </ul>
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="row g-0">
                            <div class="col-sm-6">
                                <div class="widget-card widget--success">
                                    <a href="{{ route('admin.report.merchant.transaction') }}?search={{ $merchant->username }}"
                                    ...
                                    <div class="widget-card-left">
                                        ...
                                        <div class="widget-card-content">
                                            ...
                                            <h6 class="widget-amount">
                                                {{ gs('cur_sym') }}{{ showAmount($merchant->balance, currencyFormat: false) }}
                                                <span class="currency">
                                                    {{ __(gs('cur_text')) }}
                                                </span>
                                            </h6>
                            ...
                            <div class="col-sm-6">
                                <div class="widget-card widget--success">
                                    <a href="{{ route('admin.deposit.list') }}?search={{ $merchant->username }}"
                                        class="widget-card-link"></a>
                                    <div class="widget-card-left">
                                        <div class="widget-icon">
                                        ...
                                        <div class="widget-card-content">
                                            <p class="widget-title">@lang('Received Payments')</p>
                                            <h6 class="widget-amount">
                                                {{ gs('cur_sym') }}
                                                {{ showAmount($widget['merchant_amount'], currencyFormat: false) }}
                                                <span class="currency">
                                                    {{ __(gs('cur_text')) }}
                                                </span>
                                            </h6>
                            ...
                            <div class="col-sm-6">
                                <div class="widget-card widget--warning">
                                    <a href="{{ route('admin.withdraw.data.all') }}?search={{ $merchant->username }}"
                                        class="widget-card-link"></a>
                                    <div class="widget-card-left">
                                        <div class="widget-icon">
            ...
            <form action="{{ route('admin.merchants.update', [$merchant->id]) }}" method="POST"
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-3">
                        ...
                        <x-permission_check permission="update merchant">
                            ...
                            <div class="col-lg-6 col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group input--group ">
                                        <span class="input-group-text mobile-code">+{{ $merchant->dial_code }}</span>
                                        <input type="number" name="mobile" value="{{ $merchant->mobile }}"
                                    ...
                                    <select name="country" class="form-control select2">
                                        @foreach ($countries as $key => $country)
                                            <option data-mobile_code="{{ $country->dial_code }}"
                                                value="{{ $key }}" @selected($merchant->country_code == $key)>
                            ...
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
                                ...
                                <div class="form-check form-switch form-switch-success form--switch pl-0">
                                    <input class="form-check-input" type="checkbox" role="switch"
                                        id="email_verification" name="ev" @checked($merchant->ev)>
                                </div>
                            </div>
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
                                ...
                                <div class="form-check form-switch form-switch-success form--switch pl-0">
                                    <input class="form-check-input" type="checkbox" role="switch"
                                        id="mobile_berification" name="sv" @checked($merchant->sv)>
                                </div>
                            </div>
                            ...
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
                                ...
                                <div class="form-check form-switch form-switch-success form--switch pl-0">
                                    <input class="form-check-input" type="checkbox" role="switch" id="kyc_verification"
                                        name="kv" @checked($merchant->kv == Status::KYC_VERIFIED)>
                                </div>
                            </div>
                            <div class="verification-switch__item d-flex justify-content-between align-items-center gap-2">
        ...
        <div class="col-xxl-4">
            <div class="card h-100">
                <div class="card-header d-flex justify-content-between align-items-center gap-3">
                    ...
                    <a href="{{ route('admin.report.login.history') }}?search={{ $merchant->username }}"
    ...
    <x-admin.ui.modal id="merchantStatusModal">
        <x-admin.ui.modal.header>
            <div>
                ...
                @if ($merchant->status == Status::MERCHANT_ACTIVE)
                    <small>@lang('If this merchant is banned, they will no longer have access to their dashboard.')</small>
                @else
                    <small>
                        <span class=" text--info">@lang('Ban reason was'):</span>
                        <span>{{ __($merchant->ban_reason) }}</span>
                    </small>
                @endif
            </div>
        ...
        <x-admin.ui.modal.body>
            <form action="{{ route('admin.merchants.status', $merchant->id) }}" method="POST">
                ...
                @else
                    <h4 class="mt-3 text-center text--warning">@lang('Are you sure to unban this merchant?')</h4>
                @endif
                <div class="form-group">
                    @if ($merchant->status == Status::MERCHANT_ACTIVE)
    ...
    <x-admin.ui.modal id="addSubModal">
        <x-admin.ui.modal.header>
            <div>
                ...
                <small class="modal-subtitle">@lang('Add funds to merchant accounts by entering the desired amount below')</small>
            </div>
            <button type="button" class="btn-close close" data-bs-dismiss="modal" aria-label="Close">
                <i class="las la-times"></i>
            </button>
        ...
        <x-admin.ui.modal.body>
            <form method="POST" action="{{ route('admin.merchants.add.sub.balance', $merchant->id) }}">
                @csrf
...
@push('breadcrumb-plugins')
    <div class=" d-flex gap-2  flex-wrap">
        <x-permission_check permission="update merchant balance">
        ...
        <x-permission_check permission="ban merchant">
            @if ($merchant->status == Status::MERCHANT_ACTIVE)
                <button type="button" class="flex-fill btn  btn--warning" data-bs-toggle="modal"
                    data-bs-target="#merchantStatusModal">
                    <i class="las la-ban me-1"></i>@lang('Ban Merchant')
                </button>
            @else
                <button type="button" class="flex-fill btn  btn--info" data-bs-toggle="modal"
                    data-bs-target="#merchantStatusModal">
                    <i class="las la-ban me-1"></i>@lang('Unban Merchant')
                </button>
            @endif
        </x-permission_check>
        <x-permission_check permission="view merchant notifications">
            <a href="{{ route('admin.report.merchant.notification.history') }}?search={{ $merchant->username }}"
                class="flex-fill btn  btn--secondary">
                <i class="las la-bell me-1"></i>@lang('Notifications')
            </a>
        </x-permission_check>
        <x-permission_check permission="verify merchant kyc">
