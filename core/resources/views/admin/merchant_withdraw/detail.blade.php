...
@section('panel')
    <div class="row gy-4">
                ...
                <x-admin.ui.card.body>
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap ps-0">
                            ...
                            <span class="fs-14">
                                <a href="{{ route('admin.merchants.detail', $withdrawal->merchant_id) }}">
                                    <span>@</span>{{ @$withdrawal->merchant->username }}
                                </a>
                            </span>
                        </li>
        ...
        <div class="col-lg-8 col-md-6">
            ...
            <x-admin.ui.card>
                <x-admin.ui.card.header>
                    <h4 class="card-title">@lang('Merchant Withdraw Information')</h4>
                </x-admin.ui.card.header>
                <x-admin.ui.card.body>
                    @if ($details != null)
                    ...
                    @if ($withdrawal->status == Status::PAYMENT_PENDING)
                        <div class="mt-3 d-flex gap-2 flex-wrap">
                            <x-permission_check permission="approve merchant withdraw">
                                <button class="btn btn-outline--success " data-bs-toggle="modal"
                                    data-bs-target="#approveModal">
                                    <i class="las la-check-double"></i> @lang('Approve')
                                </button>
                            </x-permission_check>
                            <x-permission_check permission="reject merchant withdraw">
    ...
    <x-admin.ui.modal id="approveModal">
        ...
        <x-admin.ui.modal.body>
            <form action="{{ route('admin.withdraw.merchant.data.approve') }}" method="POST">
                @csrf
    ...
    <x-admin.ui.modal id="rejectModal">
        ...
        <x-admin.ui.modal.body>
            <form action="{{ route('admin.withdraw.merchant.data.reject') }}" method="POST">
                @csrf
                <input type="hidden" name="id" value="{{ $withdrawal->id }}">
                <div class="form-group">
